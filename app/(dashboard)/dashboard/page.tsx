...
export default async function DashboardPage() {
  ...
  return <main className="main"><p className="eyebrow">Operational overview</p><h1>Evidence, verdicts,<br/>human decisions.</h1><p className="lede">Rules calculate every score. AI can explain persisted results, but cannot change them.</p>
    ...
    <RuleEngineExplainer rules={rules.flatMap((rule) => rule.versions[0] ? [{ code: rule.code, control: rule.control, severity: rule.severity, deduction: rule.deduction, version: rule.versions[0].version, legalReference: rule.versions[0].legalReference }] : [])} />
