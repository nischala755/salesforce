...
export default async function DashboardPage() {
  const [contacts, rules] = await Promise.all([
    ...
    prisma.complianceRule.findMany({ where: { active: true }, orderBy: { code: "asc" }, include: { versions: { where: { effectiveTo: null }, orderBy: { version: "desc" }, take: 1 } } }),
  ]);
  ...
  const activeRules=rules.flatMap((rule)=>rule.versions[0]?[{code:rule.code as RuleCode,control:rule.control,severity:rule.severity,deduction:rule.deduction,version:rule.versions[0].version,legalReference:rule.versions[0].legalReference}]:[]);
  ...
  return <main className="main"><p className="eyebrow">Operational overview</p><h1>Evidence, verdicts,<br/>human decisions.</h1><p className="lede">Rules calculate every score. AI can explain persisted results, but cannot change them.</p>
    <section className="persona-strip" aria-label="ComplyLens user personas">
      <div><span className="eyebrow">Primary persona</span><strong>DPO / Privacy Operations Lead</strong><p>Owns oversight, independently approves remediation and produces defensible evidence.</p></div>
      <div><span className="eyebrow">Daily collaborators</span><strong>CRM steward · Incident lead</strong><p>Investigate failed evidence, initiate corrections and coordinate incident records.</p></div>
      <div><span className="eyebrow">Customer-system impact</span><strong>An evidence layer, not a CRM replacement</strong><p>Existing systems remain sources of record; production connectors sync only required metadata.</p></div>
    </section>
