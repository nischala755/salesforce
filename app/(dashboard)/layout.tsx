...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="shell">
    <header className="topbar"><Link className="brand" href="/dashboard">ComplyLens</Link><nav className="nav" aria-label="Primary"><Link href="/dashboard">Overview</Link><Link href="/contacts">Contacts</Link><Link href="/breach">Incident response</Link><Link href="/rights-requests">Rights requests</Link><Link href="/timeline">Timeline</Link><Link href="/dpo">DPO oversight</Link></nav><span className="user">{session.name} · {session.role}</span><LogoutButton /></header>
