...
export async function POST(request: Request) {
  try {
    ...
    const incident = await prisma.$transaction(async (tx) => { const created = await tx.incidentLog.create({ data: { ...parsed.data, occurredAt: new Date(parsed.data.occurredAt) } }); await writeAudit(tx, { actorId: session.sub, action: "incident.created", entityType: "IncidentLog", entityId: created.id, metadata: { affectedContactCount: created.affectedContactCount } }); return created; });
